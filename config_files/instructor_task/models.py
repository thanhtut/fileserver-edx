...
class ReportStore(object):
    ...
    def from_config(cls):
        ...
        elif storage_type.lower() == "simplefileserver":  ##THAN added simplefileserver report store option
            return SimpleFileServerReportStore.from_config()
...
###THAN added simple fileserver report store to keep server files authenticated with edx users.
###The reports being stored with this has "staff" permission by default. Only accessible to users designated as staff
class SimpleFileServerReportStore(ReportStore):
    """
    SimpleFileServerReportStore implementation of a ReportStore. It serves file stored on local file system with a custom
    Django server that serves files via HTTP/HTTPS. The simplefileserver works by session cookie sharing with edx to check whether
    the user has proper authenticatin level. By default this report store keeps the file only to the staff.
    Set the root path to be "/edx/files/staff" to ensure that staff permission is applied in ..../edx-platform/lms/envs/commons.py
    Sample configuration
    "GRADES_DOWNLOAD": { "ROOT_PATH": "/edx/files/staff", "URL_ROOT": "http://files.example.com", "STORAGE_TYPE": "simplefileserver" },

    Report files will be stored at the location [ROOT_PATH]/[Course ID]/[File Name]
    """
    def __init__(self, root_path, url_root):
        """
        Initialize with root_path where we're going to store our files. We
        will build a directory structure under this for each course.
        """
    ...
    def from_config(cls):
        ...
        that there is a dict in settings named GRADES_DOWNLOAD and that it has
        a ROOT_PATH that maps to an absolute file path that the web app has
        write permissions to. `SimpleFileServerReportStore` will create any intermediate
        directories as needed. Example::

            STORAGE_TYPE : "simplefileserver"
            ROOT_PATH : /edx/files/staff/
        """
        return cls(settings.GRADES_DOWNLOAD['ROOT_PATH'],settings.GRADES_DOWNLOAD['URL_ROOT'])
    ...
    def links_for(self, course_id):
        ...
        return [
            (filename, (url_root + '/staff/' + urllib.quote(course_dir + '/' +filename)))
            for filename, full_path in files
        ]
