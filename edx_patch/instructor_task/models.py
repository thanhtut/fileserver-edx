...
class SimpleFileServerReportStore(ReportStore):
    ...
    def from_config(cls):
        ...
        """
        return cls(settings.GRADES_DOWNLOAD['ROOT_PATH'],settings.GRADES_DOWNLOAD['ROOT_URL'])
    ...
    def links_for(self, course_id):
        ...
        """
        course_dir = self.path_to(course_id, '')
        course_path = urllib.quote(course_id.to_deprecated_string(), safe='')
        if not os.path.exists(course_dir):
            return []
        files = [(filename, os.path.join(course_dir, filename)) for filename in os.listdir(course_dir)]
        files.sort(key=lambda (filename, full_path): os.path.getmtime(full_path), reverse=True)
        ...
        return [
            (filename, (self.root_url + 'staff/' + urllib.quote(course_path + '/' + filename)))
            for filename, full_path in files
        ]
